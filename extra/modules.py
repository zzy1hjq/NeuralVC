...
import extra.commons as commons
from extra.commons import get_padding, init_weights
from extra.DSConv import (
...
class TransformerCouplingLayer(nn.Module):
    def __init__(self,
        ...
        self.pre = nn.Conv1d(self.half_channels, hidden_channels, 1)
        self.enc = attentions.FFT(hidden_channels, filter_channels, n_heads, n_layers, kernel_size, p_dropout,
                                  isflow=True,
                                  gin_channels=gin_channels) if wn_sharing_parameter is None else wn_sharing_parameter
        self.post = nn.Conv1d(hidden_channels, self.half_channels * (2 - mean_only), 1)
        self.post.weight.data.zero_()
