...
import extra.attentions as attentions
...
class BNE(nn.Module):
  def __init__(self,
    ...
    self.bne = nn.Conv1d(in_channels, out_channels, 1)
...
class NeuralDec(nn.Module):
    ...
    def __init__(self, channels: int = 1, dimension: int = 128, n_filters: int = 32, n_residual_layers: int = 1,
        ...
        self.conv_pre = Conv1d(192, 512, 7, 1, padding=3)
...
class NeuralDecNoModi(nn.Module):
    """
    未修改的神经解码器
    """
    def __init__(self, channels: int = 1, dimension: int = 128, n_filters: int = 32, n_residual_layers: int = 1,
...
class HuBERT_NeuralDec_VITS(nn.Module):
  """
  使用修改过的神经解码器
  """
  def __init__(self, 
    ...
    **kwargs):
    ...
    self.enc_p = Encoder(256, inter_channels, hidden_channels, 5, 1, 16, gin_channels=gin_channels)
    self.dec = NeuralDec(dimension=512)
  ...
  def forward(self, c, spec, g=None, mel=None, c_lengths=None, spec_lengths=None):
    ...
    _, m_p, logs_p, _ = self.enc_p(c, c_lengths, g=g)#这里也输入一下g会不会更好?models_g_content.py里面有模型代码
    z, m_q, logs_q, spec_mask = self.enc_q(spec, spec_lengths, g=g) 
    z_p = self.flow(z, spec_mask, g=g)
    z_slice, ids_slice = commons.rand_slice_segments(z, spec_lengths, self.segment_size)
    o = self.dec(z_slice, g=g)
      
    return o, ids_slice, spec_mask, (z, z_p, m_p, logs_p, m_q, logs_q)#,g_hat
  ...
  def infer(self, c, g=None, mel=None, c_lengths=None):
    ...
    z_p, m_p, logs_p, c_mask = self.enc_p(c, c_lengths, g=g)
...
class HuBERT_NeuralDec_VITS_NoModi(nn.Module):
  """
  使用未修改的神经解码器
  """
  def __init__(self, 
    ...
    **kwargs):
    ...
    self.enc_p = Encoder(256, inter_channels, hidden_channels, 5, 1, 16, gin_channels=gin_channels)
    self.dec = NeuralDecNoModi(dimension=192)
    self.enc_q = Encoder(spec_channels, inter_channels, hidden_channels, 5, 1, 16, gin_channels=gin_channels) 
    self.flow = ResidualCouplingBlock(inter_channels, hidden_channels, 5, 1, 4, gin_channels=gin_channels)
